...
enum { NetSupported, NetSystemTray, NetSystemTrayOP, NetSystemTrayOrientation, NetSystemTrayVisual,
       ...
       NetSystemTrayOrientationHorz, NetWMWindowTypeDialog, NetClientList, NetWMCheck, NetDesktopNames,
...
static int drawstatusbar(Monitor *m, int bh, char* text, int systraywidth);
...
int
drawstatusbar(Monitor *m, int bh, char* stext, int systraywidth) {
	int ret, i, w, x, len;
	short isCode = 0;
	char *text;
	char *p;
...
setup(void)
{
	...
	netatom[NetSystemTrayOrientationHorz] = XInternAtom(dpy, "_NET_SYSTREM_TRAY_ORIENTATION_HORZ", False);
